	...
	public function viewAction()
{
	...
	// ===== Top countries (limit 10) =====
	 $sql = "SELECT SUM(revenue) AS revenue, country
			FROM view_analytics
			WHERE release_id = '".$release_id."' and revenue > 0 $sWhere
			GROUP BY country
			ORDER BY SUM(revenue) DESC
			LIMIT 10";
	$statement = $adapter->createStatement($sql);
	$result    = $statement->execute();
	$rowsTopCountries = (new \Zend\Db\ResultSet\ResultSet())->initialize($result)->toArray();

	$topCountries = [];
	foreach ($rowsTopCountries as $row) {
		// Get country code from country name
		$countryCode = $this->getCountryCodeFromName($row['country']);
		
		$topCountries[] = [
			'flag'  => $countryCode, // Use the country code for flag
			'name'  => $row['country'], // Use the country name from database
			'value' => (float)$row['revenue']
		];
	}
	
	// Get top 5 subscription types
		$sql = "SELECT 
					subscription_type,
					SUM(revenue) as total_revenue
				FROM view_analytics
				WHERE release_id = '".$release_id."' and  subscription_type!= '' $sWhere
		...
		$statement = $adapter->createStatement($sql);
		$result    = $statement->execute();
		$rowsSubscription = (new \Zend\Db\ResultSet\ResultSet())->initialize($result)->toArray();

		$subscription_type = [];

		if (!empty($rowsSubscription)) {
			// Calculate total revenue for these 4 subscription types
			$totalRevenue = array_sum(array_column($rowsSubscription, 'total_revenue'));
			
			// If you want percentage based on total of ALL subscription types, not just these 4
			// You would need a separate query to get total revenue from all subscription types
			
			foreach ($rowsSubscription as $row) {
				$label = ucfirst($row['subscription_type']);
				$revenue = (float)$row['total_revenue'];
				
				// Calculate percentage relative to the total of these 4 items
				$percentage = ($revenue / $totalRevenue) * 100;
				
				$width = round($percentage);
				$formattedValue = '€' . number_format($revenue, 2);
		...
		$label_month_payment_info = array();
			...
			foreach ($info as $release_id => $stores) 
			{
					foreach ($stores as $store => $sales_types) 
					{
						...
						if(!in_array($store,$ALL_STORES))
						{
							$sData = array();
							$sData['name'] = $store;
							$storeTable->insert($sData);
							$ALL_STORES[] = $store;
						}
						...
						$aData['subscription_type'] = $sales_types['subscription_type'];
						$aData['country'] = $sales_types['country'];
			...
			if($sel_type == 'believe')
			{
				...
				while (($data = fgetcsv($handle, 10000, ';')) !== FALSE) 
				{
						...
						else
						{
							...
							$country = utf8_encode(trim($data[$headerMap['country / region']]));
							$subscription_type = utf8_encode(trim($data[$headerMap['streaming subscription type']]));
							...
							{
								if(strstr($store_name,"believe") || strstr($store_name,"youtube official") || strstr($store_name,"youtube ugc")  )
								{
									$sales_type = 'View';
								}
							...
							if( ($sales_type == 'Creation' || $sales_type == 'Stream' || $sales_type == 'View' ) )
							{
								...
								if($release_id > 0 )
								{
									...
									$info[$release_id][$store]['country'] = $country;
									$info[$release_id][$store]['subscription_type'] = $subscription_type;
								}
								else
								{
									...
									if($label_id > 0)
									{
										...
										$other_release[$label_id][$store]['country'] = $country;
										$other_release[$label_id][$store]['subscription_type'] = $subscription_type;
									}
								}
							}
						}
...
{
    // Comprehensive mapping of country names to ISO 3166-1 alpha-2 codes
    $countryMapping = [
        // A
        'Afghanistan' => 'af',
        'Albania' => 'al',
    ...
    if (isset($countryMapping[$cleanName])) {
        return $countryMapping[$cleanName];
    }
    ...
    foreach ($partialMatches as $partial => $code) {
        if (strpos($lowerName, $partial) !== false) {
            return $code;
        }
    }
    
    // If still no match, generate a 2-letter code from the name
    return $this->generateCountryCode($cleanName);
}

private function generateCountryCode($countryName)
{
    // Remove non-alphabetic characters
    $clean = preg_replace('/[^a-zA-Z\s]/', '', $countryName);
    
    // Get words
    $words = preg_split('/\s+/', $clean);
    
    if (count($words) >= 2) {
        // For multi-word names, use first letters of first two words
        return substr(strtolower($words[0]), 0, 1) . substr(strtolower($words[1]), 0, 1);
    } else {
        // For single-word names, use first two letters
        return substr(strtolower($clean), 0, 2);
    }
}
