...
public function exportmetaAction()
{
    ...
    try {
        ...
        $fmtDate = function($d){
            ...
            $ts = strtotime($d); return $ts ? date('m/d/Y', $ts) : '';
        };
        $splitNames = function($s){
            $parts = preg_split('/,|&|;/u', (string)$s);
            $out=[]; foreach ($parts as $p) { $p = trim($p); if ($p!=='') $out[] = $p; }
            return $out;
        };
        ...
        $looksLikeId = function($v){
            $v = trim((string)$v);
            if ($v==='') return false;
            if (ctype_digit($v)) return true;
            if (preg_match('/^[A-Za-z0-9]{16,}$/', $v)) return true;
            return false;
        };
        ...
        $select->join('tbl_track','tbl_track.id = view_tracks.id',['trackPrice','preview_start'],'left');
        ...
        $fetchMap = function($table,$idCol,$nameCol) use ($adapter){
            $map=[]; try {
                ...
                $res=$stmt->execute();
                foreach ($res as $row) { $map[(string)$row['id']] = $row['name']; }
            } catch (\Throwable $e) {}
            return $map;
        };
        ...
        foreach ($tracks as $t) {
            ...
            $meta->setCellValue("C$r",$t['version']);
        ...
        $roleAliases = [
            ...
            'composer'=>'Composer','remixer'=>'Remixer',
        ...
        // ======= Spotify ID autofill (name match) =======
        ...
        // -------- write Contributors sheet (headers UNTOUCHED) ----------
        $ctr = $spreadsheet->getSheetByName('Contributors');
        if (!$ctr) throw new \Exception('Contributors sheet not found in template.');

        // map existing headers (row 1) -> columns (case-insensitive, newline/space normalized)
        ...
        $findColByContains = function($needle) use ($hdrCols){
            ...
            foreach ($hdrCols as $norm=>$idx) {
                if (strpos($norm,$needle) !== false) return \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($idx);
            }
            return null;
        };

        // pick columns by content, not by fixed text (don’t modify row 1 at all)
        $colReleaseTitle = $findColByContains('release title') ?: 'A';
        $colContributor  = $findColByContains('contributor name') ?: 'B';
        $colSpotifyId    = $findColByContains('spotify'); // null => skip spotify entirely

        // clear ONLY body rows
        ...
        foreach ($byReleaseContributor as $mid=>$people) {
            ...
            foreach ($people as $name=>$roles) {
                ...
                if ($colSpotifyId) {
                    ...
                    // write only valid 22-char Spotify Artist IDs
                    $ctr->setCellValue($colSpotifyId.$rowC, $isSpotifyId($sid) ? $sid : '');
                }

                // role columns: rely on whatever is already in template (E..L usually)
  ...
  public function moveToProcessingAction()
{
    ...
    if ($request->isPost()) {
        ...
        if (!empty($ids) && is_array($ids)) {
            ...
            $sql = "UPDATE tbl_release SET status = 'inreview', in_process = 1 WHERE id IN ($idList)";
...
public function fnGrid()
{
  ...
  $trackTbl = new TableGateway('tbl_track', $adapter);
     ...
     */
    $aColumns = array('id','"" as chkbx','status','cover_img','title','label_name','digitalReleaseDate','tot_tracks','isrc','upc','"" as delivery_status','assigned_team','"" as r_status','"" as r_title','releaseArtist','rejected_flag','reject_reason','version','pcn','in_process','internal_notes');
...
if (!empty($_POST['search']['value'])) {
   ...
   $sWhere = "";
    ...
    // 🕒 In review
        ...
        $row[] =  $note_color . '<div title="This release is in review." data-toggle="tooltip" data-placement="bottom">
