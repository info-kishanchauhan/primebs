...
public function linkSpotifyToArtistAction()
{
    ...
    // 🔹 NEW FLAG: create_new
    $createNew = (int)$this->params()->fromPost('create_new', 0);
    ...
    // 🟡 Case: user clicked "Create new Spotify profile"
    if ($createNew === 1) {
        // ensure we have a local artist id (create if needed)
        if ($artistId <= 0 && $artistName !== '') {
            $artistId = $this->getOrCreateArtistIdByName($artistName, $adapter);
        }

        if ($artistId <= 0) {
            return $this->json(['status'=>'ERR','message'=>'artist_id or name required']);
        }

        // mark a flag (add this column in DB if not exists)
        $hasCol = false;
        try {
            $adapter->query("ALTER TABLE tbl_artist ADD COLUMN spotify_requested TINYINT(1) DEFAULT 0", []);
            $hasCol = true;
        } catch (\Throwable $e) {
            // ignore if already exists
        }

        $gw->update(['spotify_requested' => 1, 'updated_at' => date('Y-m-d H:i:s')], ['id' => $artistId]);

        return $this->json([
            'status'     => 'OK_NEW',
            'message'    => 'New Spotify profile marked for creation.',
            'artist_id'  => $artistId,
            'artist_name'=> $artistName
        ]);
    }

    // 🟢 Normal existing Spotify link flow (unchanged)
    if ($idOrUrl === '') {
    ...
    'spotify_requested' => 0,  // ✅ reset flag once real Spotify ID linked
    ...
    public function getrecAction()
{
	...
	public function saveTrackAction()
	{
        ...
        if ($request->isPost()) {
			...
			$master_id = $rowset[0]['master_id'];
			$volume = $rowset[0]['volume'];
			$exist_order_id = $rowset[0]['order_id'];
