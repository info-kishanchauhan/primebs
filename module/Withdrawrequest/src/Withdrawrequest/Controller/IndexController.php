...
public function fnGrid()
{
    ...
    $aColumns = array(
        ...
        // 4th col -> DUE PAYMENTS (latest description from tbl_transaction)
        '(SELECT description FROM tbl_transaction t WHERE t.req_id = v.id ORDER BY t.id DESC LIMIT 1)',

        // 5th col -> MONTH YEAR
        'date_format(month_year,"%M %Y")',
    ...
    $sIndexColumn = "id";
    /* DB table to use (with alias v) */
    $sTable = "view_withdraw_request v";
    $config = $this->getServiceLocator()->get('config');
    $arrDBInfo=$config['db'];
...
public function fnGrid2()
{
    ...
    $aColumns = array(
        ...
        // DUE PAYMENTS from tbl_transaction
        '(SELECT description FROM tbl_transaction t WHERE t.req_id = v.id ORDER BY t.id DESC LIMIT 1)',
