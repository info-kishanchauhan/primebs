...
public function submittaxAction()
{
    $request = $this->getRequest();

    // POST + logged-in user check
    if (!$request->isPost() || empty($_SESSION['user_id'])) {
    ...
    $adapter = $this->getServiceLocator()->get('Zend\Db\Adapter\Adapter');
    $table   = new \Zend\Db\TableGateway\TableGateway('tbl_user_tax', $adapter);

    // Common fields
    $taxCountry     = strtolower(trim($data['tax_country']      ?? ''));
    $taxProfileType = strtolower(trim($data['tax_profile_type'] ?? 'individual'));
    $now            = date('Y-m-d H:i:s');

    // --- init all known columns (table ke columns ke hisaab se) ---
    $save = [
        'user_id'          => $user_id,
    ...
    // =============== INDIA + INDIVIDUAL (TDS) ===============
    if ($taxCountry === 'in' && $taxProfileType === 'individual') {

        $save['pan_full_name'] = trim($data['pan_full_name'] ?? '');
        $save['pan_number']    = trim($data['pan_number']    ?? '');
        $save['address']       = trim($data['address']       ?? '');
        $save['state']         = trim($data['state']         ?? '');
        $save['pincode']       = trim($data['pincode']       ?? '');

        // company related fields already null hain
    }

    // =============== INDIA + COMPANY (GST) ===============
    elseif ($taxCountry === 'in' && $taxProfileType === 'company') {

        $save['business_name']   = trim($data['business_name']    ?? '');
        $save['gstin']           = trim($data['gstin']            ?? '');
        $save['company_pan']     = trim($data['company_pan']      ?? '');
        $save['company_address'] = trim($data['company_address']  ?? '');
        $save['company_state']   = trim($data['company_state']    ?? '');
        $save['company_pincode'] = trim($data['company_pincode']  ?? '');

        // individual fields already null hain
    }

    // =============== NON-INDIA ===============
    // yahan kuch extra fields nahi chahiye, upar sab null hi rahenge

    try {
        $existing = $table->select(['user_id' => $user_id])->current();

        if ($existing) {
            $table->update($save, ['user_id' => $user_id]);
        } else {
            $save['created_at'] = $now;
            $table->insert($save);
        }
        ...
        // debugging ke liye:
        // error_log('submittaxAction ERROR: ' . $e->getMessage());
        // agar error dikhana ho to yahan flash message / log kar sakte ho
    }

    return $this->redirect()->toUrl('/settings/bankinformation?tax_saved=1');
}
...
public function bankinformationAction()
{
    ...
    /**
     * ✅ NEW: Tax information load
     */
   $taxInfo = null;
try {
    $taxTable = new \Zend\Db\TableGateway\TableGateway('tbl_user_tax', $adapter);
    $taxInfo  = $taxTable->select(['user_id' => $userID])->current();
} catch (\Exception $e) {
    $taxInfo = null;
}
...
return new \Zend\View\Model\ViewModel([
    ...
    'taxInfo'            => $taxInfo,   // 🔥 yahi naam important hai
]);
}
