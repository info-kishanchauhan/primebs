<?php
namespace Rejectreason\Controller;
...
class IndexController extends AbstractActionController
{
    ...
    public function getrecAction()
    {
        ...
        if ($request->isPost()) {
            ...
            $projectTable = new TableGateway('tbl_reject_reason', $adapter);
        ...
        if ($request->isPost()) {
            ...
            $projectTable = new TableGateway('tbl_reject_reason', $adapter);
				...
				if($request->getPost("pAction") == "ADD")
				{
					...
					//$customObj->createlog("module='Rejectreason',action='Rejectreason ".$aData['name']." Added',action_id='".$iMasterID."' ");
					$result['DBStatus'] = 'OK';
				}
				else  if($request->getPost("pAction") == "EDIT")
				{
					...
					//$customObj->createlog("module='Rejectreason',action='Rejectreason ".$aData['name']." Edited',action_id='".$iMasterID."' ");
					$result['DBStatus'] = 'OK';
				}
        }
        else
...
public function fnGrid()
{
     ...
     */
    $aColumns = array('id','tag','description');
    /* Indexed column (used for fast and accurate table cardinality) */
    $sIndexColumn = "id";
    /* DB table to use */
    $sTable = "tbl_reject_reason";
    $config = $this->getServiceLocator()->get('config');
    $arrDBInfo=$config['db'];
    /* Database connection information */
    $gaSql['rejectreason']       = $arrDBInfo['username'];
 ...
 {        
	...
	$sql="select id,tag as name from tbl_reject_reason where 1=1 $cond order by name desc";		        
	$optionalParameters=array();        
	$statement = $adapter->createStatement($sql, $optionalParameters);        
	$result = $statement->execute();        
	$resultSet = new ResultSet;        
