<?php
namespace Releasingnetwork\Controller;
...
class IndexController extends AbstractActionController
{
    ...
    public function getrecAction()
    {
        ...
        if ($request->isPost()) {
            ...
            $projectTable = new TableGateway('tbl_releasing_network', $adapter);
        ...
        if ($request->isPost()) {
            ...
            $projectTable = new TableGateway('tbl_releasing_network', $adapter);
				...
				if($request->getPost("pAction") == "ADD")
				{
					...
					//$customObj->createlog("module='Releasingnetwork',action='Releasingnetwork ".$aData['name']." Added',action_id='".$iMasterID."' ");
					$result['DBStatus'] = 'OK';
				}
				else  if($request->getPost("pAction") == "EDIT")
				{
					...
					//$customObj->createlog("module='Releasingnetwork',action='Releasingnetwork ".$aData['name']." Edited',action_id='".$iMasterID."' ");
					$result['DBStatus'] = 'OK';
				}
        }
        else
...
public function fnGrid()
{
    ...
    $sIndexColumn = "id";
    /* DB table to use */
    $sTable = "tbl_releasing_network";
    $config = $this->getServiceLocator()->get('config');
    $arrDBInfo=$config['db'];
    /* Database connection information */
    $gaSql['releasingnetwork']       = $arrDBInfo['username'];
 ...
 public function getreleasingnetworkAction()    
 {        
	 $sl = $this->getServiceLocator();        
	 $adapter = $sl->get('Zend\Db\Adapter\Adapter');     
	...
	if($_REQUEST['user_id'] > 0  && $_SESSION['STAFFUSER'] == '0')
	{
		$sql="select group_concat(releasing_network)as releasing_network from tbl_staff where id='".$_REQUEST['user_id']."' ";		        
		 $optionalParameters=array();        
		 $statement = $adapter->createStatement($sql, $optionalParameters);        
		 $result = $statement->execute();        
		 $resultSet = new ResultSet;        
		 ...
		 if($rowset[0]['releasing_network'] != '')
			$cond=" and  id in(".$rowset[0]['releasing_network'].") ";
	}
	
	 $sql="select id,name from tbl_releasing_network where 1=1 $cond order by name desc";		        
	 $optionalParameters=array();        
	 $statement = $adapter->createStatement($sql, $optionalParameters);        
	 $result = $statement->execute();        
	 $resultSet = new ResultSet;        
