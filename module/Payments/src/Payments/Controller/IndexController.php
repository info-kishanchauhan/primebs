	...
	public function sendPaymentReqAction()
{
    ...
    if ($request->isPost()) {
        ...
        // ---------- 1) SAME USER KA KOI OPEN REQUEST NA HO ----------
        $rowset3 = $reqTable->select(array("user_id='".$_SESSION['user_id']."' order by id desc limit 1"));
        $rowset3 = $rowset3->toArray();

        if (count($rowset3) > 0) {
            if ($rowset3[0]['status'] != 'Success' &&
                $rowset3[0]['status'] != 'Rejected' &&
                $rowset3[0]['status'] != 'Declined') {
        ...
        // getWithdrawBalanceAmount() se:
        //  - amount
        //  - month_year (last month included)
        //  - description  -> "Payment of Royalties for Q1 2025 > Q2 2025 ..."
        ...
        $r_id = $reqTable->lastInsertValue; // <- request id

        // ---------- 4) PAYMENT ID + FINAL DESCRIPTION (PERMANENT TEXT) ----------
        $paymentId = 'PDA'.sprintf('%03d', $r_id).'ALD'.date('Y');   // e.g. PDA024ALD2025

        $uData = array();
        $uData['payment_id'] = $paymentId;
        $reqTable->update($uData, array("id='".$r_id."'"));

        // base description jo function ne diya
        $rawDescription = isset($amount_info['description']) ? $amount_info['description'] : '';

        if ($rawDescription == '') {
            // fallback – kabhi description na aaye to
            $rawDescription = 'Payment of Royalties up to '.$amount_info['month_year'];
        }

        // YAHI FINAL TEXT HAI – isme payment id inject kar rahe hain
        // Example:
        //  "Payment of Royalties for Q1 2025 > Q2 2025"
        //   => "Payment PDA024ALD2025 of Royalties for Q1 2025 > Q2 2025"
        $dueDescription = str_replace('Payment', 'Payment '.$paymentId, $rawDescription);

        // ---------- 5) LAST BALANCE NIKALNA ----------
        $rowset2 = $transactionTable->select(function(Select $select) use ($user) {
        ...
        $lastBalance = 0;
        if (count($rowset2) > 0) {
            $lastBalance = (float)$rowset2[0]['balance'];
        }

        // ---------- 6) INVOICE NO ----------
        $rowsetSettings = $settingsTable->select()->toArray();
        $lastInvoiceNo  = $rowsetSettings[0]['last_invoice_no'];

        $invoice_no = $lastInvoiceNo + 1;
        $invoice_no = 'PDA'.date('y').sprintf('%03d', $invoice_no);

        // ---------- 7) TRANSACTION ROW INSERT (YEHI DUE PAYMENTS ME DIKHEGA) ----------
        $tData = array();
        $tData['user_id']          = $user['id'];
        $tData['req_id']           = $r_id;                 // 🔴 IMPORTANT
        $tData['invoice_no']       = $invoice_no;
        $tData['transaction_date'] = date('Y-m-d');
        $tData['transaction_type'] = 'Pending Payment';
        $tData['amount']           = $amount;
        $tData['description']      = $dueDescription;       // 🔴 DUE PAYMENTS ka text
        $tData['balance']          = $lastBalance;          // same balance
        ...
        // settings me last_invoice_no update
        $sData = array();
        $sData['last_invoice_no'] = $lastInvoiceNo + 1;
        ...
        $content = '
            ...
            <p>Best regards,<br><b>Prime Digital Arena</b></p>';

        $adminRow = $staffTable->select(array("id='0' "))->toArray();
        $customObj->sendSmtpEmail($config, $adminRow[0]['email'], 'Payment Request Submission', $content);
