        ...
        if ($request->isPost()) {
            ...
            $projectTable = new TableGateway('tbl_staff', $adapter);
				...
				if($request->getPost("pAction") == "ADD")
				{
					...
					if($aData['permission_type'] == 'Full Access')
					{
						$labels = $aData['labels'];
						if(is_array($labels))
...
public function fnGrid()
{
    ...
    $gaSql['staff']       = $arrDBInfo['username'];
   ...
   $mysqli=$customObj->dbconnection();
	...
	public   function fatal_error ( $sErrorMessage = '' )
    {
	 ...
	 public function getControlledStaffAction()    
 {        
	 $sl = $this->getServiceLocator();        
	 $adapter = $sl->get('Zend\Db\Adapter\Adapter');       

	 $sql="select id,concat(first_name,' ',last_name)as name from tbl_staff where id != 0 and permission_type='Controlled Access' order by first_name asc";		        
	 $optionalParameters=array();        
	 $statement = $adapter->createStatement($sql, $optionalParameters);        
	 $result = $statement->execute();        
	 $resultSet = new ResultSet;        
 ...
 {        
	 ...
	 $sql="select id,company_name as name from tbl_staff where id != 0 $cond order by company_name asc";		        
	 $optionalParameters=array();        
	 $statement = $adapter->createStatement($sql, $optionalParameters);        
	 $result = $statement->execute();        
	 $resultSet = new ResultSet;        
