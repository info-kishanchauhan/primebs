...
class IndexController extends AbstractActionController
{
		...
		if($_POST['sel_type'] == 'believe')
		{
			...
			foreach ($other_release as $label_id => $stores) 
			{
					foreach ($stores as $store => $sales_types) 
					{
						...
						$rowset = $analyticTable->select($aData,array("release_id = '0' ","label_id='".$label_id."' ","sales_month = '".$sales_month."'"));
						$rowset = $rowset->toArray();
						if(count($rowset) > 0)
						{
							$analyticTable->update($aData,array("release_id = '0' ","label_id='".$label_id."'","sales_month = '".$sales_month."'"));
						}
						else
						{
		...
		foreach($_POST as $key => $value)
		{
			if(substr($key,0,8) == 'revenue_')
			{
				...
				if($value == 'Hold')
				{
					...
					$rData = array();
					$rData['user_id'] = $customObj->getAssignedUserforLabel($label_id);
            ...
            if (!move_uploaded_file($file['tmp_name'], $myImagePath)) {
                ...
                $result['message1'] = 'Unable to save file![signature]';
					...
					foreach($label_month_payment_info as $label_id => $revenue)
					{
						...
						$summary_html .= '<tr><td>'.$label_info['label_name'].'</td><td>'.$label_info['company_name'].'</td><td>'.$sales_month.'</td><td>'.$royalty_amount.'</td><td><select name="revenue_'.$label_id.'" class="form-control"><option  value="Unpaid" selected>Unpaid</option><option  value="Paid">Paid</option><option  value="Hold">Hold</option></select></td></tr>';
	...
	public function getGrandStream($id,$from_month,$to_month)
	{
		...
		$sql="select sum(streams)as tot_streams from view_analytics where release_id='".$id."' and sales_month >='".$from_month."' and sales_month <='".$to_month."'  $cond ";		        
		$optionalParameters=array();        
		$statement = $adapter->createStatement($sql, $optionalParameters);        
		$result = $statement->execute();        
		$resultSet = new ResultSet;        
	...
	public function getTotalAction()
	{
		...
		if($_SESSION['user_id'] != '0')
		{
			...
			$sWhere.="  AND labels in (".$labels.") and import_payment_status !='Hold' ";
		...
		$result2['tot_creation'] = $this->formatViewCount($rowset[0]['tot_creation']).' / '.$this->formatViewCount($rowset[0]['tot_view']);
...
public function fnGrid()
{
	...
	if($_GET['from_month'] == $_GET['to_month'])
	{
		if($_SESSION['user_id'] != '0')
		{
			$cond ="  AND labels in (".$labels.") and import_payment_status !='Hold'  ";
		}
...
public   function fatal_error ( $sErrorMessage = '' )
 ...
 public function getUserRate($user_id)
 {
	...
	$sql="select * from tbl_staff where id='".$user_id."' ";		        
	$optionalParameters=array();        
	$statement = $adapter->createStatement($sql, $optionalParameters);        
	$result = $statement->execute();        
	$resultSet = new ResultSet;        
