    ...
    public function  deleteAction()
    {
        ...
        if ($request->isPost()) {
            ...
            if ($request->getPost("pAction") == "DELETE") {
				...
				$customObj->createlog("module='Analytics',action='Analytics ".$info." Deleted',action_id='".$iMasterID."' ");
        ...
        if ($request->isPost()) {
            ...
            $projectTable = new TableGateway('tbl_label', $adapter);
				...
				if($request->getPost("pAction") == "ADD")
				{
					...
					$customObj->createlog("module='Analytics',action='Analytics ".$aData['name']." Added',action_id='".$iMasterID."' ");
					$result['DBStatus'] = 'OK';
				}
				else  if($request->getPost("pAction") == "EDIT")
				{
					...
					$customObj->createlog("module='Analytics',action='Analytics ".$aData['name']." Edited',action_id='".$iMasterID."' ");
					$result['DBStatus'] = 'OK';
				}
        }
        else
...
public function fnGrid()
{
    ...
    $gaSql['analytics']       = $arrDBInfo['username'];
 ...
 {        
	 $sl = $this->getServiceLocator();        
	 $adapter = $sl->get('Zend\Db\Adapter\Adapter');        
	 $sql="select id,name from tbl_label where order by name desc";		        
	 $optionalParameters=array();        
	 $statement = $adapter->createStatement($sql, $optionalParameters);        
	 $result = $statement->execute();        
	 $resultSet = new ResultSet;        
